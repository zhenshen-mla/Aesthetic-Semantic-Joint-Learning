...
class ChannelAttentionModule(nn.Module):
    def __init__(self, channel, ratio=8):
        ...
        self.A_shared_MLP = nn.Sequential(
            nn.Conv2d(channel * 2, channel // ratio, 1, bias=False),
            nn.ReLU(),
            nn.Conv2d(channel // ratio, channel, 1, bias=False)
        )
        self.B_shared_MLP = nn.Sequential(
            nn.Conv2d(channel * 2, channel // ratio, 1, bias=False),
            nn.ReLU(),
            nn.Conv2d(channel // ratio, channel, 1, bias=False)
        )
    ...
    def forward(self, x1, x2):  # batch*channel*H*W
        ...
        w_AB = self.sigmoid(self.A_shared_MLP(w))  # batch*channel*1*1
        w_BA = self.sigmoid(self.A_shared_MLP(w))
...
class SpatialAttentionModule(nn.Module):
    def __init__(self):
        ...
        self.A_conv2d = nn.Conv2d(in_channels=2, out_channels=1, kernel_size=5, stride=1, padding=2)  # kernel_size=7 padding=3
        self.B_conv2d = nn.Conv2d(in_channels=2, out_channels=1, kernel_size=5, stride=1, padding=2)
        self.sigmoid = nn.Sigmoid()
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes1=2, num_classes2=7):
        self.inplanes = 64
    ...
    def get_10x_lr_params_sklayer(self):
    ...
    def get_10x_lr_params_sem(self):
        for m in self.named_modules():
            if 'B' in m[0] and 'afa' not in m[0]:
                if isinstance(m[1], nn.Conv2d) or isinstance(m[1], nn.BatchNorm2d) or isinstance(m[1],
                                                                                                 nn.BatchNorm1d) or isinstance(
                        m[1], nn.Linear):
                    for p in m[1].parameters():
    ...
    def get_1x_lr_params_aes(self):
        for m in self.named_modules():
            if 'A' in m[0] and 'afa' not in m[0]:
                if isinstance(m[1], nn.Conv2d) or isinstance(m[1], nn.BatchNorm2d) or isinstance(m[1],
                                                                                                 nn.BatchNorm1d) or isinstance(
                        m[1], nn.Linear):
                    for p in m[1].parameters():
